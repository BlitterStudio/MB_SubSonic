...
namespace Subsonic.Domain
{
    public sealed class SubsonicService : IDisposable
    {
        ...
        private static string NewSalt()
        {
            ...
            var cryptoRandomBuffer = new byte[15];
            NetFxCrypto.RandomNumberGenerator.GetBytes(cryptoRandomBuffer);
            return BitConverter.ToString(cryptoRandomBuffer);
        }

        private static string Md5(string saltedPassword)
        {
            //Create a byte array from source data.
            var tmpSource = Encoding.UTF8.GetBytes(saltedPassword);
            ...
            var result = hasher.HashData(tmpSource);
